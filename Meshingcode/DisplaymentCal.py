...
def solve_fea(NL, EL, E, nu, load_val, element_type):
    num_nodes = len(NL)
...
if __name__ == "__main__":
    ...
    if element_type not in ['D2QU4N', 'D2TR3N']:
        print("Loai phan tu khong hop le. Tu dong chon D2QU4N.")
        element_type = 'D2QU4N'
    ...
    E = float(input("Nhap Young's Modulus (E): "))
    nu = float(input("Nhap Poisson's Ratio (nu): "))
    load_val = float(input("Nhap Tong luc keo ngang o canh phai: "))
    
    NL, EL = uniform_mesh(d1, d2, p, m, element_type=element_type)
    
    K, U, F = solve_fea(NL, EL, E, nu, load_val, element_type)
    ...
    ax1.set_title(f"Buoc 1: Luoi ban dau ({element_type})")
    ax1.axis('equal')
    ...
    ax2.set_title(f"Buoc 2: Sau khi tinh toan (Phong dai: {scale_factor})")
    ax2.axis('equal')
